...
export class AssetDomainService {
  ...
  async getAssetPreviewInfo(assetId: string, size?: string, resolution?: string) {
    ...
    let key: string | undefined = typeof asset.thumbnailKey === 'string' ? asset.thumbnailKey : undefined;
    let contentType = 'image/jpeg';
    const variants = asset.variants && typeof asset.variants === 'object' ? asset.variants as Record<string, { key: string; contentType?: string }> : undefined;
    if (!key && (size || resolution) && variants) {
      if (size && typeof variants[size] === 'object' && variants[size] && typeof variants[size].key === 'string') {
        key = variants[size].key;
        contentType = typeof variants[size].contentType === 'string' ? variants[size].contentType : contentType;
      } else if (resolution && typeof variants[resolution] === 'object' && variants[resolution] && typeof variants[resolution].key === 'string') {
        key = variants[resolution].key;
        contentType = typeof variants[resolution].contentType === 'string' ? variants[resolution].contentType : contentType;
      }
    }
    if (!key) {
